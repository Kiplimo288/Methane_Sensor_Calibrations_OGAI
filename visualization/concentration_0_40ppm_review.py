...
ax1.scatter(timestamps[data['TGS2600CH4_RandomForest'] >= 0], data['TGS2600CH4_RandomForest'][data['TGS2600CH4_RandomForest'] >= 0], 
            marker='s', color='#ff7f0e', edgecolor='black', linewidth=0.6)

ax1.scatter(timestamps[data['TGS2611CH4_RandomForest'] >= 0], data['TGS2611CH4_RandomForest'][data['TGS2611CH4_RandomForest'] >= 0], 
...
ax1.set_title('Zoomed-In View of Aeris CH4 Measurements and TGS Predictions (0-40 ppm)')
...
ax2.scatter(timestamps[data['TGS2600CH4_RandomForest'] >= 0], data['TGS2600CH4_RandomForest'][data['TGS2600CH4_RandomForest'] >= 0], 
            marker='s', color='#ff7f0e', label='TGS2600 CH4 (ppm)', edgecolor='black', linewidth=0.6)

ax2.scatter(timestamps[data['TGS2611CH4_RandomForest'] >= 0], data['TGS2611CH4_RandomForest'][data['TGS2611CH4_RandomForest'] >= 0], 
            marker='d', color='#2ca02c', label='TGS2611 CH4 (ppm)', edgecolor='black', linewidth=0.6)

# Set y-axis limits for the full view
max_value = data[['CH4 (ppm)', 'TGS2600CH4_RandomForest', 'TGS2611CH4_RandomForest']].max().max()
ax2.set_ylim(-1, max_value * 1.1)  # Adding a buffer of 10% above the max value

# Add titles and labels for the bottom panel
ax2.set_title('Full View of CH4 Measurements and Predictions')
ax2.set_xlabel('Time')
